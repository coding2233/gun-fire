...
namespace Wanderer
{
    ...
    public class ConsoleWindow : IDebuggerWindow
    {
        ...
        public void OnDraw()
        {
            if (GUILayout.Button("Fix resource"))
            {
                DeletcFolderFiles(Application.persistentDataPath);
                Application.Quit();
            }
        ...
        private void DeletcFolderFiles(string dir)
        {
            foreach (var item in Directory.GetDirectories(dir))
            {
                DeletcFolderFiles(item);
            }
        ...
        private void ExecuteCommand(string command)
        {
            ...
            if (!_commandMethod.TryGetValue(command, out callMethod))
            {
                ...
                if (index > 0 && index < command.Length - 1)
                {
                    ...
                    {
                        List<Type> allrefTypes = new List<Type>();
                        allrefTypes.AddRange(typeof(ConsoleWindow).Assembly.GetTypes());
                        Type callType = allrefTypes.Find(x => x.FullName.Equals(fullName));
                        if (callType != null)
                        {
