...
namespace Wanderer
{
    ...
    {
        private static StringBuilder s_StrBuilder = new StringBuilder();
        ...
        {
            s_StrBuilder.Clear();
            s_StrBuilder.AppendLine($"[*] [SummaryProfiler]");
            s_StrBuilder.AppendLine("{");
            s_StrBuilder.AppendLine($"    Profiler.supported:{Profiler.supported}");
            s_StrBuilder.AppendLine($"    Profiler.enabled:{Profiler.enabled}");
            s_StrBuilder.AppendLine($"    Profiler.enableBinaryLog:{Profiler.enableBinaryLog}");
            s_StrBuilder.AppendLine($"    Profiler.logFile:{Profiler.logFile}");
            s_StrBuilder.AppendLine($"    Profiler.areaCount:{Profiler.areaCount}");
            s_StrBuilder.AppendLine($"    Profiler.maxUsedMemory:{Profiler.maxUsedMemory.ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.GetMonoUsedSizeLong():{ Profiler.GetMonoUsedSizeLong().ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.GetMonoHeapSizeLong():{ Profiler.GetMonoHeapSizeLong().ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.usedHeapSizeLong:{ Profiler.usedHeapSizeLong.ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.GetTotalAllocatedMemoryLong():{ Profiler.GetTotalAllocatedMemoryLong().ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.GetTotalReservedMemoryLong():{ Profiler.GetTotalReservedMemoryLong().ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.GetTotalUnusedReservedMemoryLong():{ Profiler.GetTotalUnusedReservedMemoryLong().ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.GetAllocatedMemoryForGraphicsDriver():{ Profiler.GetAllocatedMemoryForGraphicsDriver().ToByteLengthString()}");
            s_StrBuilder.AppendLine($"    Profiler.GetTempAllocatorSize():{ Profiler.GetTempAllocatorSize().ToByteLengthString()}");
            s_StrBuilder.AppendLine();
            s_StrBuilder.AppendLine("}");
    ...
    internal abstract class MemoryProfilerBase<T> : IDebuggerWindow where T : UnityEngine.Object
    {
        ...
        protected virtual void TakeSample()
        {
            ...
            if (_samples.Count > 0)
            {
                ...
                s_StrBuilder.Clear();
                s_StrBuilder.AppendLine($"[*] [MemoryProfiler] [{typeof(T).FullName}]");
                s_StrBuilder.AppendLine("{");
