			...
			while(trans.parent != null && loopLimiter < LOOP_MAX)
			{
				...
				if (trans.gameObject.GetComponent<ContextView>() != null)
				{
					...
					if (contextView.context != null)
					{
                        ...
                        List<IContext> contexts = new List<IContext>() { context };
						if ((context as Context).contextChildren != null)
						{
							contexts.AddRange((context as Context).contextChildren);
                        }
                        Type viewType = view.GetType();
                        foreach (var itemContext in contexts)
						{
							IMediationBinding binding = (itemContext as MVCSContext).mediationBinder.GetBinding(viewType) as IMediationBinding;
							if (binding != null)
							{
								Debug.Log($"{this.name} to context: {itemContext.GetType()}");
								if (toAdd)
								{
                                    itemContext.AddView(view);
									registeredWithContext = true;
									return;
								}
								else
								{
                                    itemContext.RemoveView(view);
									return;
								}
							}
                        }
